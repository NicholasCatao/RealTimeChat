$(document).ready(function () {
    ...
    function AppViewModel() {
        ...
        self.UsersChat = ko.observableArray([]);
        ...
        self.filter = ko.observable("");
        self.registredChatUsers = ko.computed(function () {
            if (!self.filter()) {
                return self.UsersChat();
            } else {
                return ko.utils.arrayFilter(self.UsersChat(), function (user) {
                    var fullName = user.fullName().toLowerCase();
                    return fullName.includes(self.filter().toLowerCase());
                });
            }
        ...
        self.RegistredUserList = function () {
            connection.invoke("GeRegistredUsers").then(function (result) {
                for (var i = 0; i < result.length; i++) {
                    self.UsersChat.push(new UserChat(result[i].id,
                        result[i].userName,
                        result[i].fullName,
                        result[i].avatar,
                        result[i].currentRoom,
